...
function insert_empresa($conexao, $razao_social, $cnpj, $nome_fantasia, $endereco, $email, $data_abertura, $patrimonio, $meta_total, $descricao, $summernote_introducao, $summernote_imagens, $summernote_objetivo, $summernote_video, $fk_idtermo_condicao, $fk_idusuario, $fk_idlog_projeto) {
    $stmt_empresa = $conexao->prepare("INSERT INTO projeto (razao_social, cnpj_projeto, nome_fantasia, endereco, email_corporativo, data_abertura_empresa, patrimonio_oferecido, meta_total, desc_empresa, summernote_introducao, summernote_imagens, summernote_objetivo, summernote_video, fk_idtermo_condicao, fk_idusuario, fk_idlog_projeto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt_empresa) {
        die('Erro na preparação da declaração SQL: ' . $conexao->error);
    }
    $stmt_empresa->bind_param("ssssssddssssiii", $razao_social, $cnpj, $nome_fantasia, $endereco, $email, $data_abertura, $patrimonio, $meta_total, $descricao, $summernote_introducao, $summernote_imagens, $summernote_objetivo, $summernote_video, $fk_idtermo_condicao, $fk_idusuario, $fk_idlog_projeto);
